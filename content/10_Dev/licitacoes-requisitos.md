...
|---|---|---|---|---|
| **RF01** | Importar dados públicos | `tcc ingest --de 201301 --ate 202404` baixa as 135 competências para `bronze/` e grava Parquet em `silver/`, com uma linha em `ingestao_log` por arquivo | 02 | pendente |
...
| **RF03** | Armazenar dados no banco | 11 tabelas criadas por migration. ~342 mil licitações, ~7M itens e ~21,8M participantes carregados. Reprocessar a mesma competência não duplica registros | 01, 02 | pendente |
...
| **RF10** | Registrar logs de processamento | `ingestao_log` com linhas lidas, inseridas, atualizadas e rejeitadas, mais status e mensagem de erro por arquivo. `GET /health` expõe a última ingestão | 01, 02 | pendente |

## Requisitos não funcionais

| # | Enunciado (PDF) | Critério de aceitação | Plano | Status |
|---|---|---|---|---|
| **RNF01** | Arquitetura modular | As seis regras de dependência valem e são verificáveis: `tcc-api` não importa `tcc-jobs`, o frontend não conhece o banco, `etl/` e `ml/` só são acionados por CLI. A separação em repositórios torna a violação impossível por construção | 01 | pendente |
| **RNF02** | Código de fácil manutenção | `ruff`, `Pint` e `ESLint` sem erro. CI verde nos três repositórios. Cobertura de testes de ao menos 80% em `etl/` e `ml/` | 01, 07 | pendente |
...
| **RNF04** | Banco versionado por migrations | `alembic upgrade head` e `alembic downgrade base` funcionam nos dois sentidos, com teste automatizado. Nenhuma migration do Laravel existe | 01 | pendente |
| **RNF05** | Containerização via Docker | `docker compose up -d` sobe PostgreSQL, API e dashboard. `curl /api/health` devolve `{"status":"ok","database":"ok"}` | 01 | pendente |
| **RNF06** | Tempo de resposta adequado para consultas | **Medido, não estimado.** Endpoints de consulta com p95 < 300 ms, analíticos < 500 ms, carga inicial de tela < 2 s. Carga completa das 135 competências em menos de 45 min | 03, 06 | pendente |
| **RNF07** | Segurança no acesso à aplicação | **Atendimento parcial declarado.** Segredos em variáveis de ambiente; `git ls-files \| grep '\.env$'` vazio nos cinco repositórios. Autenticação de usuários está fora de escopo | 01 | parcial |
...
**RNF02 - "código de fácil manutenção".** Manutenibilidade não se mede por opinião. Traduzido em lint sem erro, CI verde e piso de cobertura de 80% nas camadas onde bug silencioso é mais caro (`etl/`, onde um parser errado contamina toda a base, e `ml/`, onde um erro de avaliação invalida as conclusões). O piso de 80% é meta assumida, não imposição externa - vale revisar com o orientador.
